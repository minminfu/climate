...
def F(*args):
    ...
    U = args[4].value
    hsat = args[5].value
    return np.maximum(0,L*np.sqrt((u+U)*(u+U) + v*v)*(h-hsat))
...
Nu   = 0.1
D    = 0.1
tau  = 5 #Relaxation timescale for the fluid height
taus = 5 #Relaxation timescale for the tracer
h0   = 0.5
s0   = 1
...
problem.parameters['tauh'] = tauh
problem.parameters['taus'] = taus
...
problem.parameters['L'] = 0.01
problem.parameters['hsat'] = 0.4
problem.parameters['U'] = -2 # mean trade wind
...
problem.add_equation("dt(h) + h/tauh = -Q(h,u,v,L,U,hsat) + h0/tauh - h*dx(u) - h*vy - u*dx(h) - v*dy(h)") # const on RHS h^0 implicit
#problem.add_equation("dt(h)  = - h*dx(u) - h*vy - u*dx(h) - v*dy(h)")
problem.add_equation("dt(s) + s/taus - D*(dx(dx(s)) + dy(sy)) = h0/taus - s*dx(u) - s*vy - u*dx(s) - v*sy") # passive scalar
...
h['g'] = h0*xyone + eta['g'] * (3 + 6*yy**2)/4. * np.exp(-yy**2/2) / Grav
